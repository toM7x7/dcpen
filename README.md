...
- 鉛筆型ペンが空中のラックに並ぶ。既定は **14色＋虹の15本**、`penCount` で1〜15本へ調整可能
- 各ペンは色を変えても残る **P1〜P15の物理ID表示**を持ち、複数人で受け渡しても識別できる
- **VR**: 左右どちらの手でもグリップ（握る）で掴む。**両手に1本ずつ持てる**。掴んだ瞬間の持ち方のまま手に追従（縦持ち・横持ち自由）。グリップを離すと**その場の空中に浮いて留まる**
- 持ち手のトリガーで描く。`enableQuickRing` 有効時は**トリガー2連打（0.3秒以内）でペン中心のクイックリング**を開き、選択肢へペン先を1秒近づけてLINE / RIBBON / FUDE / 消しゴム / 色を確定する。無効時は従来どおり道具を順番に循環する
...
- `enableQuickRing` で、開いた地点に固定される一時リングから道具と15色を選択。色は物理ペンごとに同期し、描いたストロークへ固定保存
...
| `enableQuickRing` | `false` | トリガー2連打でペン中心クイックリングを表示 |
| `penCount` | `15` | ラックへ並べる物理ペン本数（1〜15）。色リングは常に15色 |
